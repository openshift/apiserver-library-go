...
func TestListOrderedSCCs(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := c.listOrderedSCCs(tc.requiredSCCName, tc.validatedSCCHint, tc.specMutationAllowed)
...
func (m *mockSCCLister) Get(name string) (*securityv1.SecurityContextConstraints, error) {
	...
	return nil, errors.NewNotFound(schema.GroupResource{Group: "security.openshift.io", Resource: "securitycontextconstraints"}, name)
}
