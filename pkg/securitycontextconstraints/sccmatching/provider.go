...
func (s *simpleProvider) ValidateContainerSecurityContext(pod *api.Pod, container *api.Container, fldPath *field.Path) field.ErrorList {
	...
	if s.scc.UserNamespaceLevel == securityv1.NamespaceLevelRequirePod {
		...
		if hostUsers == nil {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("hostUsers"), hostUsers, "Host Users may not be nil and must be set to false"))
		} else if *hostUsers {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("hostUsers"), hostUsers, "Host Users is not allowed to be set to true"))
		}
	}
